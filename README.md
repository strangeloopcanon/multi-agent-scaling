...
The latest live follow-up is **Phase IId: Market Calibration Intervention**. That run keeps the same six-worker market setup but changes the private bid note so workers start from a held-out calibration prior before bidding. The saved full 50-task rerun finished at **24 / 50**, versus **23 / 50** for the matched Phase IIb market rerun and **27 / 50** for the matched centralized router. We later repaired the evaluator path and reran the seven tasks that had created the old `29 -> 24` gap. That repaired slice recovered **4 / 7** tasks under the cleaned path, so the saved **24 / 50** now reads as a lower bound rather than the last word on Phase IId. The older published market result of **29 / 50** still stands as the repo's canonical Phase II benchmark figure until we rerun the full 50 cleanly under the repaired path. The detailed note lives with the other Phase II follow-ups: [Phase IIb / IId report](docs/research/report/10_PHASE_2B_CENTRAL_ROUTER_BASELINE_2026-04-07.md).
